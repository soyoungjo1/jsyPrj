...
// 이모지 추가
if (!hasEmoji) {
  // 타입 추출 (feat: 또는 feat : 형식 모두 지원)
  const typeMatch = COMMIT_MSG.match(/^([a-z]+)\s*:/);
  ...
  if (typeMatch) {
    ...
    if (emoji) {
      // 이모지 추가: "feat:" 또는 "feat :" → "✨ feat:"
      COMMIT_MSG = COMMIT_MSG.replace(/^([a-z]+)\s*:/, `${emoji} $1:`);
    }
  }
}

// commitlint 검증용 임시 메시지 (이모지 제거)
...
try {
  // 임시 파일에 깨끗한 메시지 저장 (commitlint 검증용)
  ...
  // commitlint 실패해도 이모지는 추가된 상태로 저장
  if (!hasEmoji && COMMIT_MSG !== fs.readFileSync(normalizedMsgFile, 'utf-8').trim()) {
    fs.writeFileSync(normalizedMsgFile, COMMIT_MSG, 'utf-8');
  }
